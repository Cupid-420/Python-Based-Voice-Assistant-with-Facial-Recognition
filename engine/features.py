...
def playAssistantSound():
    ...
    playsound(music_dir)
def playOpenSound():
    music_dir = "www\\assets\\audio\\solo_leveling_system.mp3"
    playsound(music_dir)
...
#Call Assistant Program (not work to go Notes)
def hotword():
    porcupine = None
    paud = None
    audio_stream = None
...
def whatsApp(mobile_no, query, flag, name):
...
# chat bot 
def chatBot(query):
    user_input = f"Answer in no more than 30 words: {query.lower()}"
